...
def write_file(name, preds):
    ...
    if not (os.path.isdir('./signal3_submission')): os.mkdir('./signal3_submission')
    if not (os.path.isdir('./signal3_submission/{}'.format(name))): os.mkdir('./signal3_submission/{}'.format(name))
    submission.to_csv('./signal3_submission/{}/submission.csv'.format(name), encoding='utf-8-sig', index=False, header=False)
...
if __name__ == "__main__":
    ...
    for fold in range(5):
        ...
        weights_path = './signal3_best_model/{}/fold{}.pt'.format(name, fold+1)
