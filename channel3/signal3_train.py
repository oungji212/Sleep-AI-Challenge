#-*- coding: utf-8 -*-
from signal3_load_preprocess import * 
from signal3_model import *
import os
...
def save_model(name, model, optimizer, scheduler, fold, best=False):
    ...
    if not (os.path.isdir('./signal3_model')): os.mkdir('./signal3_model')
    if not (os.path.isdir('./signal3_model/{}'.format(name))): os.mkdir('./signal3_model/{}'.format(name))
    torch.save(state, './signal3_model/{}/fold{}.pt'.format(name, fold+1))
    if best == True:
        if not (os.path.isdir('./signal3_best_model')): os.mkdir('./signal3_best_model')
        if not (os.path.isdir('./signal3_best_model/{}'.format(name))): os.mkdir('./signal3_best_model/{}'.format(name))
        torch.save(state, './signal3_best_model/{}/fold{}.pt'.format(name, fold+1))
