...
class TrainValDataset(Dataset):
    def __init__(self, patient_dataset, root='./yuns_dataset'): # main.py에서 device설정해주므로 여기서는 pass
        self.root = root 
    ...
    def __getitem__(self, index):
        ...
        img = np.expand_dims(img,axis=2) # (21,1920) → (21,1920,1) : HxWxC (numpy ver.) cf.tensor: CxHxW
...
class TestDataset(Dataset):
    def __init__(self, root='./yuns_dataset'): # main.py에서 device설정해주므로 여기서는 pass
        self.root = root 
        self.data = {} # 최종적으로 data dictionary안에 data['image'], data['label'] 존재
