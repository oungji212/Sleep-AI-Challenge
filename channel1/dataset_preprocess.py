...
def save_preprocessed_imgs(which_data='train'):
    ...
    for patient in tqdm(patients_unique,desc='{} img preprocess'.format(which_data)):
        ...
        if not os.path.isdir('./yuns_dataset/{}'.format(patient)): # 환자별 폴더 만들기
            os.mkdir('./yuns_dataset/{}'.format(patient)) # os.makedirs의 경우 permission denied 발생

        for idx in range(len(patient_img)):
            img_paths = imgpath_together_ls[idx]
            if len(img_paths) <= 3:
                for no_img_num in range(4-len(img_paths)):
                    globals()['img{}'.format(no_img_num)] = np.zeros((21,480))
                for img_num in range(4-len(img_paths),4):
                    img_path = img_paths[img_num-(4-len(img_paths))]
                    img = np.array(Image.open(img_path).crop((0,75,480,96)))  # 실제 이미지의 경우 기본적으로 1차원!
                    globals()['img{}'.format(img_num)] = img 
            else:
                for i in range(4):
                    path = img_paths[i]
                    img = np.array(Image.open(path).crop((0,75,480,96))) # EOG 부분만 잘라내기
                    globals()['img{}'.format(img_num)] = img 
            ...
            save_img.save('./yuns_dataset/{}/{}'.format(patient,unique_patient_data[1][idx])) # 기준 epoch 사진의 원본 이름으로 저장. 저장 위치만 다른 것임


if __name__ == '__main__':
    # USER에서 실행
    if not os.path.isdir('./yuns_dataset'): 
        os.mkdir('./yuns_dataset') # os.makedirs의 경우 permission denied 발생
